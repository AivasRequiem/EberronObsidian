---
obsidianUIMode: preview
cssclasses: json5e-note
tags:
- ttrpg-cli/compendium/src/5e/xphb
aliases:
- Chapter 7: Spells
---
# Chapter 7: Spells
*Source: Player's Handbook (2024), p. 235* 

![Archmages of Greyhawk—Jall...](Інструменти%20ДМ/CLI/books/players-handbook-2024/img/181-08-001-jallarzi-mordenkainen-bigby.webp#center "Archmages of Greyhawk—Jallarzi, Mordenkainen, and Bigby—prepare their magic as they open a gate to another plane")

This chapter gives rules for casting spells. It also includes "descriptions" of common spells in the worlds of Dungeons & Dragons. Those spells are used by many class features, magic items, and monsters.

## Gaining Spells

Before you can cast a spell, you must have the spell prepared in your mind or have access to the spell from a magic item, such as a [Spell Scroll](Інструменти%20ДМ/CLI/items/spell-scroll-xdmg.md). Your features specify which spells you have access to, if any; whether you always have certain spells prepared; and whether you can change the list of spells you have prepared.

### Preparing Spells

If you have a list of level 1+ spells you prepare, your spellcasting feature specifies when you can change the list and the number of spells you can change, as summarized in the Spell Preparation by Class table.

![Preparing Spells; Spell Preparation by Class](Інструменти%20ДМ/CLI/tables/preparing-spells-spell-preparation-by-class-xphb.md)

Most spellcasting monsters don't change their lists of prepared spells, but the DM is free to alter them.

### Always-Prepared Spells

Certain features might give you a spell that you always have prepared. If you also have a list of prepared spells that you can change, a spell that you always have prepared doesn't count against the number of spells on that list.

> [!note] Casting in Armor
> 
> You must have training with any armor you are wearing to cast spells while wearing it. You are otherwise too hampered by the armor for spellcasting.
^casting-in-armor

## Casting Spells

Each "spell description" has a series of entries that provide the details needed to cast the spell. The following sections explain each of those entries, which follow a spell's name.

### Spell Level

Every spell has a level from 0 to 9, which is indicated in a spell's description. A spell's level is an indicator of how powerful it is. Cantrips—simple spells that can be cast almost by rote—are level 0. The rules for each spellcasting class say when its members gain access to spells of certain levels.

#### Spell Slots

Spellcasting is taxing, so a spellcaster can cast only a limited number of level 1+ spells before resting. Spell slots are the main way a spellcaster's magical potential is represented. Each spellcasting class gives its members a limited number of spell slots of certain spell levels. For example, a level 3 Wizard has four level 1 spell slots and two level 2 slots.

When you cast a spell, you expend a slot of that spell's level or higher, effectively "filling" a slot with the spell. Imagine a spell slot is a groove of a certain size—small for a level 1 slot and larger for a higher-level spell. A level 1 spell fits into a slot of any size, but a level 2 spell fits only into a slot that's at least level 2. So when a level 3 Wizard casts [Magic Missile](Інструменти%20ДМ/CLI/spells/magic-missile-xphb.md), a level 1 spell, that Wizard spends one of four level 1 slots and has three remaining.

Finishing a [Long Rest](Інструменти%20ДМ/CLI/rules/variant-rules/long-rest-xphb.md) restores any expended spell slots.

#### Casting without Slots

There are several ways to cast a spell without expending a spell slot:

- **Cantrips.** A cantrip is cast without a spell slot.  
- **Rituals.** Certain spells have the Ritual tag in the Casting Time entry. Such a spell can be cast following the normal rules for spellcasting, or it can be cast as a Ritual. The Ritual version of a spell takes 10 minutes longer to cast than normal, but it doesn't expend a spell slot. To cast a spell as a Ritual, a spellcaster must have it prepared.  
- **Special Abilities.** Some characters and monsters have special abilities that allow them to cast specific spells without a spell slot. This casting is usually limited in another way, such as being able to cast the spell a limited number of times per day.  
- **Magic Items.** [Spell Scrolls](Інструменти%20ДМ/CLI/items/spell-scroll-xdmg.md) and some other magic items contain spells that can be cast without a spell slot. The description of such an item specifies how many times a spell can be cast from it.  

#### Using a Higher-Level Spell Slot

When a spellcaster casts a spell using a slot that is of a higher level than the spell, the spell takes on the higher level for that casting. For instance, if a Wizard casts [Magic Missile](Інструменти%20ДМ/CLI/spells/magic-missile-xphb.md) using a level 2 slot, that *Magic Missile* is level 2. Effectively, the spell expands to fill the slot it is put into.

Some spells, such as [Magic Missile](Інструменти%20ДМ/CLI/spells/magic-missile-xphb.md) and [Cure Wounds](Інструменти%20ДМ/CLI/spells/cure-wounds-xphb.md), have more powerful effects when cast at a higher level, as detailed in a spell's description.

### School of Magic

Each spell belongs to a school of magic. The schools are listed in the Schools of Magic table. These categories help describe spells but have no rules of their own, although some other rules refer to them.

![School of Magic; Schools of Magic](Інструменти%20ДМ/CLI/tables/school-of-magic-schools-of-magic-xphb.md)

### Class Spell Lists

If a spell is on a class's spell list, the class's name appears in parentheses after the spell's school of magic. Some features add a spell to a character's spell list even if the character isn't a member of a class in the parentheses.

### Casting Time

Most spells require the [Magic](Інструменти%20ДМ/CLI/rules/actions.md#Magic) action to cast, but some spells require a Bonus Action, a Reaction, or 1 minute or more. A spell's Casting Time entry specifies which of those is required.

#### One Spell with a Spell Slot per Turn

On a turn, you can expend only one spell slot to cast a spell. This rule means you can't, for example, cast a spell with a spell slot using the [Magic](Інструменти%20ДМ/CLI/rules/actions.md#Magic) action and another one using a Bonus Action on the same turn.

#### Reaction and Bonus Action Triggers

A spell that has a casting time of a Reaction is cast in response to a trigger that is defined in the spell's Casting Time entry. Some spells that have a casting time of a Bonus Action are also cast in response to a trigger defined in the spell.

#### Longer Casting Times

Certain spells—including a spell cast as a [Ritual](Інструменти%20ДМ/CLI/rules/variant-rules/ritual-xphb.md)—require more time to cast: minutes or even hours. While you cast a spell with a casting time of 1 minute or more, you must take the [Magic](Інструменти%20ДМ/CLI/rules/actions.md#Magic) action on each of your turns, and you must maintain [Concentration](Інструменти%20ДМ/CLI/rules/conditions.md#Concentration) while you do so. If your Concentration is broken, the spell fails, but you don't expend a spell slot. To cast the spell again, you must start over.

### Range

A spell's range indicates how far from the spellcaster the spell's effect can originate, and the spell's description specifies which part of the effect is limited by the range.

A range usually takes one of the following forms:

- **Distance.** The range is expressed in feet.  
- **Touch.** The spell's effect originates on something the spellcaster must touch, as defined in the spell.  
- **Self.** The spell is cast on the spellcaster or emanates from them, as specified in the spell.  

If a spell has movable effects, they aren't restricted by its range unless the spell's description says otherwise.

### Components

![An aasimar Wizard uses a c...](Інструменти%20ДМ/CLI/books/players-handbook-2024/img/182-08-002-aasimar-wizard.webp#center "An aasimar Wizard uses a crystal material component to focus the magic of Cone of Cold")

A spell's components are physical requirements the spellcaster must meet to cast the spell. Each spell's description indicates whether it requires Verbal (V), Somatic (S), or Material (M) components. If the spellcaster can't provide one or more of a spell's components, the spellcaster can't cast the spell.

#### Verbal (V)

A Verbal component is the chanting of esoteric words that sound like nonsense to the uninitiated. The words must be uttered in a normal speaking voice. The words themselves aren't the source of the spell's power; rather, the particular combination of sounds, with specific pitch and resonance, sets the threads of magic in motion. Thus, a creature who is gagged or in an area of magical silence can't cast a spell with a Verbal component.

> [!note] Creating Verbal Components
> 
> If you'd like to say a spell's Verbal component, you may make up the words. However you make them, the goal is to create something that's easy to say and that doesn't mean anything in the real world. Consider this method: take the name of the spell, keep only one instance of each of its letters, and rearrange the remaining letters into words. For example, remove the second *l* from [Fireball](Інструменти%20ДМ/CLI/spells/fireball-xphb.md) and rearrange the remaining letters to create *Ber Fila* or*Fel Bira*.
^creating-verbal-components

#### Somatic (S)

A Somatic component is a forceful gesticulation or an intricate set of gestures. A spellcaster must use at least one of their hands to perform these movements.

#### Material (M)

A Material component is a particular material used in a spell's casting, as specified in parentheses in the Components entry. These materials aren't consumed by the spell unless the spell's description states otherwise. The spellcaster must have a hand free to access them, but it can be the same hand used to perform Somatic components, if any.

If a spell doesn't consume its materials and doesn't specify a cost for them, a spellcaster can use a [Component Pouch](Інструменти%20ДМ/CLI/items/component-pouch-xphb.md) (see "chapter 6") instead of providing the materials specified in the spell, or the spellcaster can substitute a Spellcasting Focus if the caster has a feature that allows that substitution. To use a Component Pouch, you must have a hand free to reach into it, and to use a Spellcasting Focus, you must hold it unless its description says otherwise (see "chapter 6" for descriptions).

### Duration

A spell's duration is the length of time the spell persists after it is cast. A duration typically takes one of the following forms:

- **Concentration.** A duration that requires Concentration follows the [Concentration](Інструменти%20ДМ/CLI/rules/conditions.md#Concentration) rules.  
- **Instantaneous.** An instantaneous duration means the spell's magic appears only for a moment and then disappears.  
- **Time Span.** A duration that provides a time span specifies how long the spell lasts in rounds, minutes, hours, or the like. For example, a Duration entry might say "1 minute," meaning the spell ends after 1 minute has passed. While a time-span spell that you cast is ongoing, you can dismiss it (no action required) if you don't have the [Incapacitated](Інструменти%20ДМ/CLI/rules/conditions.md#Incapacitated) condition.  

### Effects

The effects of a spell are detailed after its duration entry. Those details present exactly what the spell does, which ignores mundane physical laws; any outcomes beyond those effects are under the DM's purview. Whatever the effects, they typically deal with targets, saving throws, attack rolls, or all three, each of which is detailed below.

#### Targets

A typical spell requires the caster to pick one or more targets to be affected by the spell's magic. A spell's description says whether the spell targets creatures, objects, or something else.

##### A Clear Path to the Target

To target something with a spell, a caster must have a clear path to it, so it can't be behind [Total Cover](Інструменти%20ДМ/CLI/rules/variant-rules/cover-xphb.md).

##### Targeting Yourself

If a spell targets a creature of your choice, you can choose yourself unless the creature must be [Hostile](Інструменти%20ДМ/CLI/rules/variant-rules/hostile-attitude-xphb.md) or specifically a creature other than you.

##### Areas of Effect

Some spells, such as [Thunderwave](Інструменти%20ДМ/CLI/spells/thunderwave-xphb.md), cover an area called an [area of effect](Інструменти%20ДМ/CLI/rules/variant-rules/area-of-effect-xphb.md), which is defined in the "rules glossary". The area determines what the spell targets. The description of a spell specifies whether it has an area of effect, which is typically one of these shapes: [Cone](Інструменти%20ДМ/CLI/rules/variant-rules/cone-area-of-effect-xphb.md), [Cube](Інструменти%20ДМ/CLI/rules/variant-rules/cube-area-of-effect-xphb.md), [Cylinder](Інструменти%20ДМ/CLI/rules/variant-rules/cylinder-area-of-effect-xphb.md), [Emanation](Інструменти%20ДМ/CLI/rules/variant-rules/emanation-area-of-effect-xphb.md), [Line](Інструменти%20ДМ/CLI/rules/variant-rules/line-area-of-effect-xphb.md), or [Sphere](Інструменти%20ДМ/CLI/rules/variant-rules/sphere-area-of-effect-xphb.md).

![](Інструменти%20ДМ/CLI/books/players-handbook-2024/img/183-08-003-areas-of-effect.webp#center)

##### Awareness of Being Targeted

Unless a spell has a perceptible effect, a creature doesn't know it was targeted by the spell. An effect like lightning is obvious, but a more subtle effect, such as an attempt to read thoughts, goes unnoticed unless a spell's description says otherwise.

##### Invalid Targets

If you cast a spell on someone or something that can't be affected by it, nothing happens to that target, but if you used a spell slot to cast the spell, the slot is still expended.

If the spell normally has no effect on a target that succeeds on a saving throw, the invalid target appears to have succeeded on its saving throw, even though it didn't attempt one (giving no hint that the creature is an invalid target). Otherwise, you perceive that the spell did nothing to the target.

#### Saving Throws

Many spells specify that a target makes a saving throw to avoid some or all of a spell's effects. The spell specifies the ability that the target uses for the save and what happens on a success or failure. Here's how to calculate the DC for your spells:

<span class='abilityDc'>**Spell save DC**: Spellcasting + Proficiency Bonus</span>

#### Attack Rolls

Some spells require the caster to make an attack roll to determine whether the spell hits a target. Here's how to calculate the attack modifier for your spells:

<span class='abilityAttackMod'>**Spell attack modifier**: Spellcasting + Proficiency Bonus</span>

#### Combining Spell Effects

The effects of different spells add together while their durations overlap. In contrast, the effects of the same spell cast multiple times don't combine. Instead, the most potent effect—such as the highest bonus—from those castings applies while their durations overlap. The most recent effect applies if the castings are equally potent and their durations overlap. For example, if two Clerics cast [Bless](Інструменти%20ДМ/CLI/spells/bless-xphb.md) on the same target, that target gains the spell's benefit only once; the target doesn't receive two bonus dice. But if the durations of the spells overlap, the effect continues until the duration of the second *Bless* ends.

> [!note] Identifying an Ongoing Spell
> 
> You can try to identify a non-instantaneous spell by its observable effects if its duration is ongoing. To identify it, you must take the [Study](Інструменти%20ДМ/CLI/rules/actions.md#Study) action and succeed on a DC 15 Intelligence ([Arcana](Інструменти%20ДМ/CLI/rules/skills.md#Arcana)) check.
^identifying-an-ongoing-spell

## Spell Descriptions

The spells are presented in alphabetical order.

- [Acid Splash](Інструменти%20ДМ/CLI/spells/acid-splash-xphb.md)  
- [Aid](Інструменти%20ДМ/CLI/spells/aid-xphb.md)  
- [Alarm](Інструменти%20ДМ/CLI/spells/alarm-xphb.md)  
- [Alter Self](Інструменти%20ДМ/CLI/spells/alter-self-xphb.md)  
- [Animal Friendship](Інструменти%20ДМ/CLI/spells/animal-friendship-xphb.md)  
- [Animal Messenger](Інструменти%20ДМ/CLI/spells/animal-messenger-xphb.md)  
- [Animal Shapes](Інструменти%20ДМ/CLI/spells/animal-shapes-xphb.md)  
- [Animate Dead](Інструменти%20ДМ/CLI/spells/animate-dead-xphb.md)  
- [Animate Objects](Інструменти%20ДМ/CLI/spells/animate-objects-xphb.md)  
- [Antilife Shell](Інструменти%20ДМ/CLI/spells/antilife-shell-xphb.md)  
- [Antimagic Field](Інструменти%20ДМ/CLI/spells/antimagic-field-xphb.md)  
- [Antipathy/Sympathy](Інструменти%20ДМ/CLI/spells/antipathy-sympathy-xphb.md)  
- [Arcane Eye](Інструменти%20ДМ/CLI/spells/arcane-eye-xphb.md)  
- [Arcane Gate](Інструменти%20ДМ/CLI/spells/arcane-gate-xphb.md)  
- [Arcane Lock](Інструменти%20ДМ/CLI/spells/arcane-lock-xphb.md)  
- [Arcane Vigor](Інструменти%20ДМ/CLI/spells/arcane-vigor-xphb.md)  
- [Armor of Agathys](Інструменти%20ДМ/CLI/spells/armor-of-agathys-xphb.md)  
- [Arms of Hadar](Інструменти%20ДМ/CLI/spells/arms-of-hadar-xphb.md)  
- [Astral Projection](Інструменти%20ДМ/CLI/spells/astral-projection-xphb.md)  
- [Augury](Інструменти%20ДМ/CLI/spells/augury-xphb.md)  
- [Aura of Life](Інструменти%20ДМ/CLI/spells/aura-of-life-xphb.md)  
- [Aura of Purity](Інструменти%20ДМ/CLI/spells/aura-of-purity-xphb.md)  
- [Aura of Vitality](Інструменти%20ДМ/CLI/spells/aura-of-vitality-xphb.md)  
- [Awaken](Інструменти%20ДМ/CLI/spells/awaken-xphb.md)  

- [Bane](Інструменти%20ДМ/CLI/spells/bane-xphb.md)  
- [Banishing Smite](Інструменти%20ДМ/CLI/spells/banishing-smite-xphb.md)  
- [Banishment](Інструменти%20ДМ/CLI/spells/banishment-xphb.md)  
- [Barkskin](Інструменти%20ДМ/CLI/spells/barkskin-xphb.md)  
- [Beacon of Hope](Інструменти%20ДМ/CLI/spells/beacon-of-hope-xphb.md)  
- [Beast Sense](Інструменти%20ДМ/CLI/spells/beast-sense-xphb.md)  
- [Befuddlement](Інструменти%20ДМ/CLI/spells/befuddlement-xphb.md)  
- [Bestow Curse](Інструменти%20ДМ/CLI/spells/bestow-curse-xphb.md)  
- [Bigby's Hand](Інструменти%20ДМ/CLI/spells/bigbys-hand-xphb.md)  
- [Blade Barrier](Інструменти%20ДМ/CLI/spells/blade-barrier-xphb.md)  
- [Blade Ward](Інструменти%20ДМ/CLI/spells/blade-ward-xphb.md)  
- [Bless](Інструменти%20ДМ/CLI/spells/bless-xphb.md)  
- [Blight](Інструменти%20ДМ/CLI/spells/blight-xphb.md)  
- [Blinding Smite](Інструменти%20ДМ/CLI/spells/blinding-smite-xphb.md)  
- [Blindness/Deafness](Інструменти%20ДМ/CLI/spells/blindness-deafness-xphb.md)  
- [Blink](Інструменти%20ДМ/CLI/spells/blink-xphb.md)  
- [Blur](Інструменти%20ДМ/CLI/spells/blur-xphb.md)  
- [Burning Hands](Інструменти%20ДМ/CLI/spells/burning-hands-xphb.md)  

- [Call Lightning](Інструменти%20ДМ/CLI/spells/call-lightning-xphb.md)  
- [Calm Emotions](Інструменти%20ДМ/CLI/spells/calm-emotions-xphb.md)  
- [Chain Lightning](Інструменти%20ДМ/CLI/spells/chain-lightning-xphb.md)  
- [Charm Monster](Інструменти%20ДМ/CLI/spells/charm-monster-xphb.md)  
- [Charm Person](Інструменти%20ДМ/CLI/spells/charm-person-xphb.md)  
- [Chill Touch](Інструменти%20ДМ/CLI/spells/chill-touch-xphb.md)  
- [Chromatic Orb](Інструменти%20ДМ/CLI/spells/chromatic-orb-xphb.md)  
- [Circle of Death](Інструменти%20ДМ/CLI/spells/circle-of-death-xphb.md)  
- [Circle of Power](Інструменти%20ДМ/CLI/spells/circle-of-power-xphb.md)  
- [Clairvoyance](Інструменти%20ДМ/CLI/spells/clairvoyance-xphb.md)  
- [Clone](Інструменти%20ДМ/CLI/spells/clone-xphb.md)  
- [Cloud of Daggers](Інструменти%20ДМ/CLI/spells/cloud-of-daggers-xphb.md)  
- [Cloudkill](Інструменти%20ДМ/CLI/spells/cloudkill-xphb.md)  
- [Color Spray](Інструменти%20ДМ/CLI/spells/color-spray-xphb.md)  
- [Command](Інструменти%20ДМ/CLI/spells/command-xphb.md)  
- [Commune](Інструменти%20ДМ/CLI/spells/commune-xphb.md)  
- [Commune with Nature](Інструменти%20ДМ/CLI/spells/commune-with-nature-xphb.md)  
- [Compelled Duel](Інструменти%20ДМ/CLI/spells/compelled-duel-xphb.md)  
- [Comprehend Languages](Інструменти%20ДМ/CLI/spells/comprehend-languages-xphb.md)  
- [Compulsion](Інструменти%20ДМ/CLI/spells/compulsion-xphb.md)  
- [Cone of Cold](Інструменти%20ДМ/CLI/spells/cone-of-cold-xphb.md)  
- [Confusion](Інструменти%20ДМ/CLI/spells/confusion-xphb.md)  
- [Conjure Animals](Інструменти%20ДМ/CLI/spells/conjure-animals-xphb.md)  
- [Conjure Barrage](Інструменти%20ДМ/CLI/spells/conjure-barrage-xphb.md)  
- [Conjure Celestial](Інструменти%20ДМ/CLI/spells/conjure-celestial-xphb.md)  
- [Conjure Elemental](Інструменти%20ДМ/CLI/spells/conjure-elemental-xphb.md)  
- [Conjure Fey](Інструменти%20ДМ/CLI/spells/conjure-fey-xphb.md)  
- [Conjure Minor Elementals](Інструменти%20ДМ/CLI/spells/conjure-minor-elementals-xphb.md)  
- [Conjure Volley](Інструменти%20ДМ/CLI/spells/conjure-volley-xphb.md)  
- [Conjure Woodland Beings](Інструменти%20ДМ/CLI/spells/conjure-woodland-beings-xphb.md)  
- [Contact Other Plane](Інструменти%20ДМ/CLI/spells/contact-other-plane-xphb.md)  
- [Contagion](Інструменти%20ДМ/CLI/spells/contagion-xphb.md)  
- [Contingency](Інструменти%20ДМ/CLI/spells/contingency-xphb.md)  
- [Continual Flame](Інструменти%20ДМ/CLI/spells/continual-flame-xphb.md)  
- [Control Water](Інструменти%20ДМ/CLI/spells/control-water-xphb.md)  
- [Control Weather](Інструменти%20ДМ/CLI/spells/control-weather-xphb.md)  
- [Cordon of Arrows](Інструменти%20ДМ/CLI/spells/cordon-of-arrows-xphb.md)  
- [Counterspell](Інструменти%20ДМ/CLI/spells/counterspell-xphb.md)  
- [Create Food and Water](Інструменти%20ДМ/CLI/spells/create-food-and-water-xphb.md)  
- [Create or Destroy Water](Інструменти%20ДМ/CLI/spells/create-or-destroy-water-xphb.md)  
- [Create Undead](Інструменти%20ДМ/CLI/spells/create-undead-xphb.md)  
- [Creation](Інструменти%20ДМ/CLI/spells/creation-xphb.md)  
- [Crown of Madness](Інструменти%20ДМ/CLI/spells/crown-of-madness-xphb.md)  
- [Crusader's Mantle](Інструменти%20ДМ/CLI/spells/crusaders-mantle-xphb.md)  
- [Cure Wounds](Інструменти%20ДМ/CLI/spells/cure-wounds-xphb.md)  

- [Dancing Lights](Інструменти%20ДМ/CLI/spells/dancing-lights-xphb.md)  
- [Darkness](Інструменти%20ДМ/CLI/spells/darkness-xphb.md)  
- [Darkvision](Інструменти%20ДМ/CLI/spells/darkvision-xphb.md)  
- [Daylight](Інструменти%20ДМ/CLI/spells/daylight-xphb.md)  
- [Death Ward](Інструменти%20ДМ/CLI/spells/death-ward-xphb.md)  
- [Delayed Blast Fireball](Інструменти%20ДМ/CLI/spells/delayed-blast-fireball-xphb.md)  
- [Demiplane](Інструменти%20ДМ/CLI/spells/demiplane-xphb.md)  
- [Destructive Wave](Інструменти%20ДМ/CLI/spells/destructive-wave-xphb.md)  
- [Detect Evil and Good](Інструменти%20ДМ/CLI/spells/detect-evil-and-good-xphb.md)  
- [Detect Magic](Інструменти%20ДМ/CLI/spells/detect-magic-xphb.md)  
- [Detect Poison and Disease](Інструменти%20ДМ/CLI/spells/detect-poison-and-disease-xphb.md)  
- [Detect Thoughts](Інструменти%20ДМ/CLI/spells/detect-thoughts-xphb.md)  
- [Dimension Door](Інструменти%20ДМ/CLI/spells/dimension-door-xphb.md)  
- [Disguise Self](Інструменти%20ДМ/CLI/spells/disguise-self-xphb.md)  
- [Disintegrate](Інструменти%20ДМ/CLI/spells/disintegrate-xphb.md)  
- [Dispel Evil and Good](Інструменти%20ДМ/CLI/spells/dispel-evil-and-good-xphb.md)  
- [Dispel Magic](Інструменти%20ДМ/CLI/spells/dispel-magic-xphb.md)  
- [Dissonant Whispers](Інструменти%20ДМ/CLI/spells/dissonant-whispers-xphb.md)  
- [Divination](Інструменти%20ДМ/CLI/spells/divination-xphb.md)  
- [Divine Favor](Інструменти%20ДМ/CLI/spells/divine-favor-xphb.md)  
- [Divine Smite](Інструменти%20ДМ/CLI/spells/divine-smite-xphb.md)  
- [Divine Word](Інструменти%20ДМ/CLI/spells/divine-word-xphb.md)  
- [Dominate Beast](Інструменти%20ДМ/CLI/spells/dominate-beast-xphb.md)  
- [Dominate Monster](Інструменти%20ДМ/CLI/spells/dominate-monster-xphb.md)  
- [Dominate Person](Інструменти%20ДМ/CLI/spells/dominate-person-xphb.md)  
- [Dragon's Breath](Інструменти%20ДМ/CLI/spells/dragons-breath-xphb.md)  
- [Drawmij's Instant Summons](Інструменти%20ДМ/CLI/spells/drawmijs-instant-summons-xphb.md)  
- [Dream](Інструменти%20ДМ/CLI/spells/dream-xphb.md)  
- [Druidcraft](Інструменти%20ДМ/CLI/spells/druidcraft-xphb.md)  

- [Earthquake](Інструменти%20ДМ/CLI/spells/earthquake-xphb.md)  
- [Eldritch Blast](Інструменти%20ДМ/CLI/spells/eldritch-blast-xphb.md)  
- [Elemental Weapon](Інструменти%20ДМ/CLI/spells/elemental-weapon-xphb.md)  
- [Elementalism](Інструменти%20ДМ/CLI/spells/elementalism-xphb.md)  
- [Enhance Ability](Інструменти%20ДМ/CLI/spells/enhance-ability-xphb.md)  
- [Enlarge/Reduce](Інструменти%20ДМ/CLI/spells/enlarge-reduce-xphb.md)  
- [Ensnaring Strike](Інструменти%20ДМ/CLI/spells/ensnaring-strike-xphb.md)  
- [Entangle](Інструменти%20ДМ/CLI/spells/entangle-xphb.md)  
- [Enthrall](Інструменти%20ДМ/CLI/spells/enthrall-xphb.md)  
- [Etherealness](Інструменти%20ДМ/CLI/spells/etherealness-xphb.md)  
- [Evard's Black Tentacles](Інструменти%20ДМ/CLI/spells/evards-black-tentacles-xphb.md)  
- [Expeditious Retreat](Інструменти%20ДМ/CLI/spells/expeditious-retreat-xphb.md)  
- [Eyebite](Інструменти%20ДМ/CLI/spells/eyebite-xphb.md)  

- [Fabricate](Інструменти%20ДМ/CLI/spells/fabricate-xphb.md)  
- [Faerie Fire](Інструменти%20ДМ/CLI/spells/faerie-fire-xphb.md)  
- [False Life](Інструменти%20ДМ/CLI/spells/false-life-xphb.md)  
- [Fear](Інструменти%20ДМ/CLI/spells/fear-xphb.md)  
- [Feather Fall](Інструменти%20ДМ/CLI/spells/feather-fall-xphb.md)  
- [Feign Death](Інструменти%20ДМ/CLI/spells/feign-death-xphb.md)  
- [Find Familiar](Інструменти%20ДМ/CLI/spells/find-familiar-xphb.md)  
- [Find Steed](Інструменти%20ДМ/CLI/spells/find-steed-xphb.md)  
- [Find the Path](Інструменти%20ДМ/CLI/spells/find-the-path-xphb.md)  
- [Find Traps](Інструменти%20ДМ/CLI/spells/find-traps-xphb.md)  
- [Finger of Death](Інструменти%20ДМ/CLI/spells/finger-of-death-xphb.md)  
- [Fire Bolt](Інструменти%20ДМ/CLI/spells/fire-bolt-xphb.md)  
- [Fire Shield](Інструменти%20ДМ/CLI/spells/fire-shield-xphb.md)  
- [Fire Storm](Інструменти%20ДМ/CLI/spells/fire-storm-xphb.md)  
- [Fireball](Інструменти%20ДМ/CLI/spells/fireball-xphb.md)  
- [Flame Blade](Інструменти%20ДМ/CLI/spells/flame-blade-xphb.md)  
- [Flame Strike](Інструменти%20ДМ/CLI/spells/flame-strike-xphb.md)  
- [Flaming Sphere](Інструменти%20ДМ/CLI/spells/flaming-sphere-xphb.md)  
- [Flesh to Stone](Інструменти%20ДМ/CLI/spells/flesh-to-stone-xphb.md)  
- [Fly](Інструменти%20ДМ/CLI/spells/fly-xphb.md)  
- [Fog Cloud](Інструменти%20ДМ/CLI/spells/fog-cloud-xphb.md)  
- [Forbiddance](Інструменти%20ДМ/CLI/spells/forbiddance-xphb.md)  
- [Forcecage](Інструменти%20ДМ/CLI/spells/forcecage-xphb.md)  
- [Foresight](Інструменти%20ДМ/CLI/spells/foresight-xphb.md)  
- [Fount of Moonlight](Інструменти%20ДМ/CLI/spells/fount-of-moonlight-xphb.md)  
- [Freedom of Movement](Інструменти%20ДМ/CLI/spells/freedom-of-movement-xphb.md)  
- [Friends](Інструменти%20ДМ/CLI/spells/friends-xphb.md)  

- [Gaseous Form](Інструменти%20ДМ/CLI/spells/gaseous-form-xphb.md)  
- [Gate](Інструменти%20ДМ/CLI/spells/gate-xphb.md)  
- [Geas](Інструменти%20ДМ/CLI/spells/geas-xphb.md)  
- [Gentle Repose](Інструменти%20ДМ/CLI/spells/gentle-repose-xphb.md)  
- [Giant Insect](Інструменти%20ДМ/CLI/spells/giant-insect-xphb.md)  
- [Glibness](Інструменти%20ДМ/CLI/spells/glibness-xphb.md)  
- [Globe of Invulnerability](Інструменти%20ДМ/CLI/spells/globe-of-invulnerability-xphb.md)  
- [Glyph of Warding](Інструменти%20ДМ/CLI/spells/glyph-of-warding-xphb.md)  
- [Goodberry](Інструменти%20ДМ/CLI/spells/goodberry-xphb.md)  
- [Grasping Vine](Інструменти%20ДМ/CLI/spells/grasping-vine-xphb.md)  
- [Grease](Інструменти%20ДМ/CLI/spells/grease-xphb.md)  
- [Greater Invisibility](Інструменти%20ДМ/CLI/spells/greater-invisibility-xphb.md)  
- [Greater Restoration](Інструменти%20ДМ/CLI/spells/greater-restoration-xphb.md)  
- [Guardian of Faith](Інструменти%20ДМ/CLI/spells/guardian-of-faith-xphb.md)  
- [Guards and Wards](Інструменти%20ДМ/CLI/spells/guards-and-wards-xphb.md)  
- [Guidance](Інструменти%20ДМ/CLI/spells/guidance-xphb.md)  
- [Guiding Bolt](Інструменти%20ДМ/CLI/spells/guiding-bolt-xphb.md)  
- [Gust of Wind](Інструменти%20ДМ/CLI/spells/gust-of-wind-xphb.md)  

- [Hail of Thorns](Інструменти%20ДМ/CLI/spells/hail-of-thorns-xphb.md)  
- [Hallow](Інструменти%20ДМ/CLI/spells/hallow-xphb.md)  
- [Hallucinatory Terrain](Інструменти%20ДМ/CLI/spells/hallucinatory-terrain-xphb.md)  
- [Harm](Інструменти%20ДМ/CLI/spells/harm-xphb.md)  
- [Haste](Інструменти%20ДМ/CLI/spells/haste-xphb.md)  
- [Heal](Інструменти%20ДМ/CLI/spells/heal-xphb.md)  
- [Healing Word](Інструменти%20ДМ/CLI/spells/healing-word-xphb.md)  
- [Heat Metal](Інструменти%20ДМ/CLI/spells/heat-metal-xphb.md)  
- [Hellish Rebuke](Інструменти%20ДМ/CLI/spells/hellish-rebuke-xphb.md)  
- [Heroes' Feast](Інструменти%20ДМ/CLI/spells/heroes-feast-xphb.md)  
- [Heroism](Інструменти%20ДМ/CLI/spells/heroism-xphb.md)  
- [Hex](Інструменти%20ДМ/CLI/spells/hex-xphb.md)  
- [Hold Monster](Інструменти%20ДМ/CLI/spells/hold-monster-xphb.md)  
- [Hold Person](Інструменти%20ДМ/CLI/spells/hold-person-xphb.md)  
- [Holy Aura](Інструменти%20ДМ/CLI/spells/holy-aura-xphb.md)  
- [Hunger of Hadar](Інструменти%20ДМ/CLI/spells/hunger-of-hadar-xphb.md)  
- [Hunter's Mark](Інструменти%20ДМ/CLI/spells/hunters-mark-xphb.md)  
- [Hypnotic Pattern](Інструменти%20ДМ/CLI/spells/hypnotic-pattern-xphb.md)  

- [Ice Knife](Інструменти%20ДМ/CLI/spells/ice-knife-xphb.md)  
- [Ice Storm](Інструменти%20ДМ/CLI/spells/ice-storm-xphb.md)  
- [Identify](Інструменти%20ДМ/CLI/spells/identify-xphb.md)  
- [Illusory Script](Інструменти%20ДМ/CLI/spells/illusory-script-xphb.md)  
- [Imprisonment](Інструменти%20ДМ/CLI/spells/imprisonment-xphb.md)  
- [Incendiary Cloud](Інструменти%20ДМ/CLI/spells/incendiary-cloud-xphb.md)  
- [Inflict Wounds](Інструменти%20ДМ/CLI/spells/inflict-wounds-xphb.md)  
- [Insect Plague](Інструменти%20ДМ/CLI/spells/insect-plague-xphb.md)  
- [Invisibility](Інструменти%20ДМ/CLI/spells/invisibility-xphb.md)  

- [Jallarzi's Storm of Radiance](Інструменти%20ДМ/CLI/spells/jallarzis-storm-of-radiance-xphb.md)  
- [Jump](Інструменти%20ДМ/CLI/spells/jump-xphb.md)  

- [Knock](Інструменти%20ДМ/CLI/spells/knock-xphb.md)  

- [Legend Lore](Інструменти%20ДМ/CLI/spells/legend-lore-xphb.md)  
- [Leomund's Secret Chest](Інструменти%20ДМ/CLI/spells/leomunds-secret-chest-xphb.md)  
- [Leomund's Tiny Hut](Інструменти%20ДМ/CLI/spells/leomunds-tiny-hut-xphb.md)  
- [Lesser Restoration](Інструменти%20ДМ/CLI/spells/lesser-restoration-xphb.md)  
- [Levitate](Інструменти%20ДМ/CLI/spells/levitate-xphb.md)  
- [Light](Інструменти%20ДМ/CLI/spells/light-xphb.md)  
- [Lightning Arrow](Інструменти%20ДМ/CLI/spells/lightning-arrow-xphb.md)  
- [Lightning Bolt](Інструменти%20ДМ/CLI/spells/lightning-bolt-xphb.md)  
- [Locate Animals or Plants](Інструменти%20ДМ/CLI/spells/locate-animals-or-plants-xphb.md)  
- [Locate Creature](Інструменти%20ДМ/CLI/spells/locate-creature-xphb.md)  
- [Locate Object](Інструменти%20ДМ/CLI/spells/locate-object-xphb.md)  
- [Longstrider](Інструменти%20ДМ/CLI/spells/longstrider-xphb.md)  

- [Mage Armor](Інструменти%20ДМ/CLI/spells/mage-armor-xphb.md)  
- [Mage Hand](Інструменти%20ДМ/CLI/spells/mage-hand-xphb.md)  
- [Magic Circle](Інструменти%20ДМ/CLI/spells/magic-circle-xphb.md)  
- [Magic Jar](Інструменти%20ДМ/CLI/spells/magic-jar-xphb.md)  
- [Magic Missile](Інструменти%20ДМ/CLI/spells/magic-missile-xphb.md)  
- [Magic Mouth](Інструменти%20ДМ/CLI/spells/magic-mouth-xphb.md)  
- [Magic Weapon](Інструменти%20ДМ/CLI/spells/magic-weapon-xphb.md)  
- [Major Image](Інструменти%20ДМ/CLI/spells/major-image-xphb.md)  
- [Mass Cure Wounds](Інструменти%20ДМ/CLI/spells/mass-cure-wounds-xphb.md)  
- [Mass Heal](Інструменти%20ДМ/CLI/spells/mass-heal-xphb.md)  
- [Mass Healing Word](Інструменти%20ДМ/CLI/spells/mass-healing-word-xphb.md)  
- [Mass Suggestion](Інструменти%20ДМ/CLI/spells/mass-suggestion-xphb.md)  
- [Maze](Інструменти%20ДМ/CLI/spells/maze-xphb.md)  
- [Meld into Stone](Інструменти%20ДМ/CLI/spells/meld-into-stone-xphb.md)  
- [Melf's Acid Arrow](Інструменти%20ДМ/CLI/spells/melfs-acid-arrow-xphb.md)  
- [Mending](Інструменти%20ДМ/CLI/spells/mending-xphb.md)  
- [Message](Інструменти%20ДМ/CLI/spells/message-xphb.md)  
- [Meteor Swarm](Інструменти%20ДМ/CLI/spells/meteor-swarm-xphb.md)  
- [Mind Blank](Інструменти%20ДМ/CLI/spells/mind-blank-xphb.md)  
- [Mind Sliver](Інструменти%20ДМ/CLI/spells/mind-sliver-xphb.md)  
- [Mind Spike](Інструменти%20ДМ/CLI/spells/mind-spike-xphb.md)  
- [Minor Illusion](Інструменти%20ДМ/CLI/spells/minor-illusion-xphb.md)  
- [Mirage Arcane](Інструменти%20ДМ/CLI/spells/mirage-arcane-xphb.md)  
- [Mirror Image](Інструменти%20ДМ/CLI/spells/mirror-image-xphb.md)  
- [Mislead](Інструменти%20ДМ/CLI/spells/mislead-xphb.md)  
- [Misty Step](Інструменти%20ДМ/CLI/spells/misty-step-xphb.md)  
- [Modify Memory](Інструменти%20ДМ/CLI/spells/modify-memory-xphb.md)  
- [Moonbeam](Інструменти%20ДМ/CLI/spells/moonbeam-xphb.md)  
- [Mordenkainen's Faithful Hound](Інструменти%20ДМ/CLI/spells/mordenkainens-faithful-hound-xphb.md)  
- [Mordenkainen's Magnificent Mansion](Інструменти%20ДМ/CLI/spells/mordenkainens-magnificent-mansion-xphb.md)  
- [Mordenkainen's Private Sanctum](Інструменти%20ДМ/CLI/spells/mordenkainens-private-sanctum-xphb.md)  
- [Mordenkainen's Sword](Інструменти%20ДМ/CLI/spells/mordenkainens-sword-xphb.md)  
- [Move Earth](Інструменти%20ДМ/CLI/spells/move-earth-xphb.md)  

- [Nondetection](Інструменти%20ДМ/CLI/spells/nondetection-xphb.md)  
- [Nystul's Magic Aura](Інструменти%20ДМ/CLI/spells/nystuls-magic-aura-xphb.md)  

- [Otiluke's Freezing Sphere](Інструменти%20ДМ/CLI/spells/otilukes-freezing-sphere-xphb.md)  
- [Otiluke's Resilient Sphere](Інструменти%20ДМ/CLI/spells/otilukes-resilient-sphere-xphb.md)  
- [Otto's Irresistible Dance](Інструменти%20ДМ/CLI/spells/ottos-irresistible-dance-xphb.md)  

- [Pass without Trace](Інструменти%20ДМ/CLI/spells/pass-without-trace-xphb.md)  
- [Passwall](Інструменти%20ДМ/CLI/spells/passwall-xphb.md)  
- [Phantasmal Force](Інструменти%20ДМ/CLI/spells/phantasmal-force-xphb.md)  
- [Phantasmal Killer](Інструменти%20ДМ/CLI/spells/phantasmal-killer-xphb.md)  
- [Phantom Steed](Інструменти%20ДМ/CLI/spells/phantom-steed-xphb.md)  
- [Planar Ally](Інструменти%20ДМ/CLI/spells/planar-ally-xphb.md)  
- [Planar Binding](Інструменти%20ДМ/CLI/spells/planar-binding-xphb.md)  
- [Plane Shift](Інструменти%20ДМ/CLI/spells/plane-shift-xphb.md)  
- [Plant Growth](Інструменти%20ДМ/CLI/spells/plant-growth-xphb.md)  
- [Poison Spray](Інструменти%20ДМ/CLI/spells/poison-spray-xphb.md)  
- [Polymorph](Інструменти%20ДМ/CLI/spells/polymorph-xphb.md)  
- [Power Word Fortify](Інструменти%20ДМ/CLI/spells/power-word-fortify-xphb.md)  
- [Power Word Heal](Інструменти%20ДМ/CLI/spells/power-word-heal-xphb.md)  
- [Power Word Kill](Інструменти%20ДМ/CLI/spells/power-word-kill-xphb.md)  
- [Power Word Stun](Інструменти%20ДМ/CLI/spells/power-word-stun-xphb.md)  
- [Prayer of Healing](Інструменти%20ДМ/CLI/spells/prayer-of-healing-xphb.md)  
- [Prestidigitation](Інструменти%20ДМ/CLI/spells/prestidigitation-xphb.md)  
- [Prismatic Spray](Інструменти%20ДМ/CLI/spells/prismatic-spray-xphb.md)  
- [Prismatic Wall](Інструменти%20ДМ/CLI/spells/prismatic-wall-xphb.md)  
- [Produce Flame](Інструменти%20ДМ/CLI/spells/produce-flame-xphb.md)  
- [Programmed Illusion](Інструменти%20ДМ/CLI/spells/programmed-illusion-xphb.md)  
- [Project Image](Інструменти%20ДМ/CLI/spells/project-image-xphb.md)  
- [Protection from Energy](Інструменти%20ДМ/CLI/spells/protection-from-energy-xphb.md)  
- [Protection from Evil and Good](Інструменти%20ДМ/CLI/spells/protection-from-evil-and-good-xphb.md)  
- [Protection from Poison](Інструменти%20ДМ/CLI/spells/protection-from-poison-xphb.md)  
- [Purify Food and Drink](Інструменти%20ДМ/CLI/spells/purify-food-and-drink-xphb.md)  

- [Raise Dead](Інструменти%20ДМ/CLI/spells/raise-dead-xphb.md)  
- [Rary's Telepathic Bond](Інструменти%20ДМ/CLI/spells/rarys-telepathic-bond-xphb.md)  
- [Ray of Enfeeblement](Інструменти%20ДМ/CLI/spells/ray-of-enfeeblement-xphb.md)  
- [Ray of Frost](Інструменти%20ДМ/CLI/spells/ray-of-frost-xphb.md)  
- [Ray of Sickness](Інструменти%20ДМ/CLI/spells/ray-of-sickness-xphb.md)  
- [Regenerate](Інструменти%20ДМ/CLI/spells/regenerate-xphb.md)  
- [Reincarnate](Інструменти%20ДМ/CLI/spells/reincarnate-xphb.md)  
- [Remove Curse](Інструменти%20ДМ/CLI/spells/remove-curse-xphb.md)  
- [Resistance](Інструменти%20ДМ/CLI/spells/resistance-xphb.md)  
- [Resurrection](Інструменти%20ДМ/CLI/spells/resurrection-xphb.md)  
- [Reverse Gravity](Інструменти%20ДМ/CLI/spells/reverse-gravity-xphb.md)  
- [Revivify](Інструменти%20ДМ/CLI/spells/revivify-xphb.md)  
- [Rope Trick](Інструменти%20ДМ/CLI/spells/rope-trick-xphb.md)  

- [Sacred Flame](Інструменти%20ДМ/CLI/spells/sacred-flame-xphb.md)  
- [Sanctuary](Інструменти%20ДМ/CLI/spells/sanctuary-xphb.md)  
- [Scorching Ray](Інструменти%20ДМ/CLI/spells/scorching-ray-xphb.md)  
- [Scrying](Інструменти%20ДМ/CLI/spells/scrying-xphb.md)  
- [Searing Smite](Інструменти%20ДМ/CLI/spells/searing-smite-xphb.md)  
- [See Invisibility](Інструменти%20ДМ/CLI/spells/see-invisibility-xphb.md)  
- [Seeming](Інструменти%20ДМ/CLI/spells/seeming-xphb.md)  
- [Sending](Інструменти%20ДМ/CLI/spells/sending-xphb.md)  
- [Sequester](Інструменти%20ДМ/CLI/spells/sequester-xphb.md)  
- [Shapechange](Інструменти%20ДМ/CLI/spells/shapechange-xphb.md)  
- [Shatter](Інструменти%20ДМ/CLI/spells/shatter-xphb.md)  
- [Shield](Інструменти%20ДМ/CLI/spells/shield-xphb.md)  
- [Shield of Faith](Інструменти%20ДМ/CLI/spells/shield-of-faith-xphb.md)  
- [Shillelagh](Інструменти%20ДМ/CLI/spells/shillelagh-xphb.md)  
- [Shining Smite](Інструменти%20ДМ/CLI/spells/shining-smite-xphb.md)  
- [Shocking Grasp](Інструменти%20ДМ/CLI/spells/shocking-grasp-xphb.md)  
- [Silence](Інструменти%20ДМ/CLI/spells/silence-xphb.md)  
- [Silent Image](Інструменти%20ДМ/CLI/spells/silent-image-xphb.md)  
- [Simulacrum](Інструменти%20ДМ/CLI/spells/simulacrum-xphb.md)  
- [Sleep](Інструменти%20ДМ/CLI/spells/sleep-xphb.md)  
- [Sleet Storm](Інструменти%20ДМ/CLI/spells/sleet-storm-xphb.md)  
- [Slow](Інструменти%20ДМ/CLI/spells/slow-xphb.md)  
- [Sorcerous Burst](Інструменти%20ДМ/CLI/spells/sorcerous-burst-xphb.md)  
- [Spare the Dying](Інструменти%20ДМ/CLI/spells/spare-the-dying-xphb.md)  
- [Speak with Animals](Інструменти%20ДМ/CLI/spells/speak-with-animals-xphb.md)  
- [Speak with Dead](Інструменти%20ДМ/CLI/spells/speak-with-dead-xphb.md)  
- [Speak with Plants](Інструменти%20ДМ/CLI/spells/speak-with-plants-xphb.md)  
- [Spider Climb](Інструменти%20ДМ/CLI/spells/spider-climb-xphb.md)  
- [Spike Growth](Інструменти%20ДМ/CLI/spells/spike-growth-xphb.md)  
- [Spirit Guardians](Інструменти%20ДМ/CLI/spells/spirit-guardians-xphb.md)  
- [Spiritual Weapon](Інструменти%20ДМ/CLI/spells/spiritual-weapon-xphb.md)  
- [Staggering Smite](Інструменти%20ДМ/CLI/spells/staggering-smite-xphb.md)  
- [Starry Wisp](Інструменти%20ДМ/CLI/spells/starry-wisp-xphb.md)  
- [Steel Wind Strike](Інструменти%20ДМ/CLI/spells/steel-wind-strike-xphb.md)  
- [Stinking Cloud](Інструменти%20ДМ/CLI/spells/stinking-cloud-xphb.md)  
- [Stone Shape](Інструменти%20ДМ/CLI/spells/stone-shape-xphb.md)  
- [Stoneskin](Інструменти%20ДМ/CLI/spells/stoneskin-xphb.md)  
- [Storm of Vengeance](Інструменти%20ДМ/CLI/spells/storm-of-vengeance-xphb.md)  
- [Suggestion](Інструменти%20ДМ/CLI/spells/suggestion-xphb.md)  
- [Summon Aberration](Інструменти%20ДМ/CLI/spells/summon-aberration-xphb.md)  
- [Summon Beast](Інструменти%20ДМ/CLI/spells/summon-beast-xphb.md)  
- [Summon Celestial](Інструменти%20ДМ/CLI/spells/summon-celestial-xphb.md)  
- [Summon Construct](Інструменти%20ДМ/CLI/spells/summon-construct-xphb.md)  
- [Summon Dragon](Інструменти%20ДМ/CLI/spells/summon-dragon-xphb.md)  
- [Summon Elemental](Інструменти%20ДМ/CLI/spells/summon-elemental-xphb.md)  
- [Summon Fey](Інструменти%20ДМ/CLI/spells/summon-fey-xphb.md)  
- [Summon Fiend](Інструменти%20ДМ/CLI/spells/summon-fiend-xphb.md)  
- [Summon Undead](Інструменти%20ДМ/CLI/spells/summon-undead-xphb.md)  
- [Sunbeam](Інструменти%20ДМ/CLI/spells/sunbeam-xphb.md)  
- [Sunburst](Інструменти%20ДМ/CLI/spells/sunburst-xphb.md)  
- [Swift Quiver](Інструменти%20ДМ/CLI/spells/swift-quiver-xphb.md)  
- [Symbol](Інструменти%20ДМ/CLI/spells/symbol-xphb.md)  
- [Synaptic Static](Інструменти%20ДМ/CLI/spells/synaptic-static-xphb.md)  

- [Tasha's Bubbling Cauldron](Інструменти%20ДМ/CLI/spells/tashas-bubbling-cauldron-xphb.md)  
- [Tasha's Hideous Laughter](Інструменти%20ДМ/CLI/spells/tashas-hideous-laughter-xphb.md)  
- [Telekinesis](Інструменти%20ДМ/CLI/spells/telekinesis-xphb.md)  
- [Telepathy](Інструменти%20ДМ/CLI/spells/telepathy-xphb.md)  
- [Teleport](Інструменти%20ДМ/CLI/spells/teleport-xphb.md)  
- [Teleportation Circle](Інструменти%20ДМ/CLI/spells/teleportation-circle-xphb.md)  
- [Tenser's Floating Disk](Інструменти%20ДМ/CLI/spells/tensers-floating-disk-xphb.md)  
- [Thaumaturgy](Інструменти%20ДМ/CLI/spells/thaumaturgy-xphb.md)  
- [Thorn Whip](Інструменти%20ДМ/CLI/spells/thorn-whip-xphb.md)  
- [Thunderclap](Інструменти%20ДМ/CLI/spells/thunderclap-xphb.md)  
- [Thunderous Smite](Інструменти%20ДМ/CLI/spells/thunderous-smite-xphb.md)  
- [Thunderwave](Інструменти%20ДМ/CLI/spells/thunderwave-xphb.md)  
- [Time Stop](Інструменти%20ДМ/CLI/spells/time-stop-xphb.md)  
- [Toll the Dead](Інструменти%20ДМ/CLI/spells/toll-the-dead-xphb.md)  
- [Tongues](Інструменти%20ДМ/CLI/spells/tongues-xphb.md)  
- [Transport via Plants](Інструменти%20ДМ/CLI/spells/transport-via-plants-xphb.md)  
- [Tree Stride](Інструменти%20ДМ/CLI/spells/tree-stride-xphb.md)  
- [True Polymorph](Інструменти%20ДМ/CLI/spells/true-polymorph-xphb.md)  
- [True Resurrection](Інструменти%20ДМ/CLI/spells/true-resurrection-xphb.md)  
- [True Seeing](Інструменти%20ДМ/CLI/spells/true-seeing-xphb.md)  
- [True Strike](Інструменти%20ДМ/CLI/spells/true-strike-xphb.md)  
- [Tsunami](Інструменти%20ДМ/CLI/spells/tsunami-xphb.md)  

- [Unseen Servant](Інструменти%20ДМ/CLI/spells/unseen-servant-xphb.md)  

- [Vampiric Touch](Інструменти%20ДМ/CLI/spells/vampiric-touch-xphb.md)  
- [Vicious Mockery](Інструменти%20ДМ/CLI/spells/vicious-mockery-xphb.md)  
- [Vitriolic Sphere](Інструменти%20ДМ/CLI/spells/vitriolic-sphere-xphb.md)  

- [Wall of Fire](Інструменти%20ДМ/CLI/spells/wall-of-fire-xphb.md)  
- [Wall of Force](Інструменти%20ДМ/CLI/spells/wall-of-force-xphb.md)  
- [Wall of Ice](Інструменти%20ДМ/CLI/spells/wall-of-ice-xphb.md)  
- [Wall of Stone](Інструменти%20ДМ/CLI/spells/wall-of-stone-xphb.md)  
- [Wall of Thorns](Інструменти%20ДМ/CLI/spells/wall-of-thorns-xphb.md)  
- [Warding Bond](Інструменти%20ДМ/CLI/spells/warding-bond-xphb.md)  
- [Water Breathing](Інструменти%20ДМ/CLI/spells/water-breathing-xphb.md)  
- [Water Walk](Інструменти%20ДМ/CLI/spells/water-walk-xphb.md)  
- [Web](Інструменти%20ДМ/CLI/spells/web-xphb.md)  
- [Weird](Інструменти%20ДМ/CLI/spells/weird-xphb.md)  
- [Wind Walk](Інструменти%20ДМ/CLI/spells/wind-walk-xphb.md)  
- [Wind Wall](Інструменти%20ДМ/CLI/spells/wind-wall-xphb.md)  
- [Wish](Інструменти%20ДМ/CLI/spells/wish-xphb.md)  
- [Witch Bolt](Інструменти%20ДМ/CLI/spells/witch-bolt-xphb.md)  
- [Word of Radiance](Інструменти%20ДМ/CLI/spells/word-of-radiance-xphb.md)  
- [Word of Recall](Інструменти%20ДМ/CLI/spells/word-of-recall-xphb.md)  
- [Wrathful Smite](Інструменти%20ДМ/CLI/spells/wrathful-smite-xphb.md)  

- [Yolande's Regal Presence](Інструменти%20ДМ/CLI/spells/yolandes-regal-presence-xphb.md)  

- [Zone of Truth](Інструменти%20ДМ/CLI/spells/zone-of-truth-xphb.md)